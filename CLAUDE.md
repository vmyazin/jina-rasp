...
This is an insurance broker directory for Fortaleza, Brazil, built as a lightweight web application. The project uses Jina AI for automated data collection, Supabase as the database, and vanilla HTML/CSS/JavaScript for the frontend. It features a modern, search-first UX inspired by travel booking sites like Travelocity.

## Architecture

### Frontend Architecture
- **Single Page Application**: Uses vanilla JavaScript with no frameworks
- **Production Files**: 
  - `index_production.html` - Main website with Travelocity-inspired UI
  - `app_production.js` - JavaScript with on-demand search and Supabase integration
  - `styles.css` - Travel industry design system with CSS custom properties
...
query = query.or(`name.ilike.%${term}%,email.ilike.%${term}%,address.ilike.%${term}%,neighborhood.ilike.%${term}%`);
